...
//    $trail->parent( 'home' );
    ...
    if ($level1) {
        $trail->push(!is_null(UserCategory::whereSlug($level1)->firstOrFail()->name)
            ? UserCategory::whereSlug($level1)->firstOrFail()->name : '',
            route('directory', $level1));
    }
    if ($level2) {
        $trail->push(!is_null(UserCategory::whereSlug($level2)->firstOrFail()->name)
            ?UserCategory::whereSlug($level2)->firstOrFail()->name:'',
