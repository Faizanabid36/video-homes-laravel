...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-2 my-3">
            <img class="w-100" src='{{$user->user_extra->profile_picture ?? 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTMgrxYAqZF6-kdFuLQesPwdAyonhn93LsxvKXax0vzbCCGd_wQ&usqp=CAU' }}' />
            ...
            <hr />
            <img src='{{$user->user_extra->company_logo ?? 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTMgrxYAqZF6-kdFuLQesPwdAyonhn93LsxvKXax0vzbCCGd_wQ&usqp=CAU' }}' style="width:100%;">
            <div class="caption-container">
                <p id="caption">{{$user->user_extra->company_name}}</p>
            </div>
        </div>
        <div class="col-md-{{count($related_videos) ? 7 : 10}} player-video mt-0">
            <div class="video-player pt_video_player " id="pt_video_player">
                <span class="mejs__offscreen"></span>
                @if($video)
                <video id="my-video_html5" style="width: 100%; height: 451.872px; position: relative;" poster="{{asset("storage/$video->thumbnail")}}" preload="none">
                    @if($video->{'8k'})
                    <source src="{{asset("storage/".str_replace('240p','4320p',$video->stream_path))}}" type="video/mp4" data-quality="4320p" title="4320p" label="4320p" res="4320">
                    @endif
                    @if($video->{'4K'})
                    <source src="{{asset("storage/".str_replace('240p','2160p',$video->stream_path))}}" type="video/mp4" data-quality="2160p" title="2160p" label="2160p" res="2160">
                    @endif
                    @if($video->{'1440p'})
                    <source src="{{asset("storage/".str_replace('240p','1440p',$video->stream_path))}}" type="video/mp4" data-quality="1440p" title="1440p" label="1440p" res="1440">
                    @endif
                    @if($video->{'1080p'})
                    <source src="{{asset("storage/".str_replace('240p','1080p',$video->stream_path))}}" type="video/mp4" dataQuality="1080p" data-quality="1080p" quality-text="1080" title="1080p" label="1080p" res="1080">
                    @endif
                    @if($video->{'720p'})
                    <source src="{{asset("storage/".str_replace('240p','720p',$video->stream_path))}}" type="video/mp4" data-quality="720p" title="720p" label="720p" res="720">
                    @endif
                    @if($video->{'480p'})
                    <source src="{{asset("storage/".str_replace('240p','480p',$video->stream_path))}}" type="video/mp4" data-quality="480p" title="480p" label="480p" res="480">
                    @endif

                    @if($video->{'360p'})
                    <source src="{{asset("storage/".str_replace('240p','360p',$video->stream_path))}}" type="video/mp4" data-quality="360p" title="360p" label="360p" res="360">
                    @endif
                    <source src="{{asset("storage/$video->stream_path")}}" type="video/mp4" data-quality="240p" title="240p" label="240p" res="240">
                ...
                <div class="video-options pt_mn_wtch_opts pt-4">
                    ...
                    <a class="btn btn-outline-success" href="{{url("dashboard#/edit_video/$video->video_id")}}">
                    ...
                    <a href="{{url('dashboard#/analytics')}}" class="btn btn-outline-danger">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    ...
                    @endif
                    @if(auth()->check() && ($video->user_id!=auth()->user()->id))
                    <button data-toggle="modal" data-target="#report" class="btn btn-primary btn-report pull-right" onclick="" data-rep="1">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M14.4,6L14,4H5V21H7V14H12.6L13,16H20V6H14.4Z"></path>
                        </svg>
                        ...
                        <div class="card w-100">
                            <div class="card-body">
                                <div class="input-group mb-3">
                                    <input type="text" value="@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="form-control input-md copylink" readonly="">
                                    <div class="input-group-prepend">
                                        <button class="btn btn-primary copylink"><i class="fa fa-link"> Copy</i>
                                        </button>
                                    </div>
                                </div>
                                <button data-url="https://www.facebook.com/sharer/sharer.php?u=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="btn btn-primary share-social"><i class="fa fa-facebook"></i>
                                </button>
                                <button data-url="https://wa.me/?text={{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}}" class="btn btn-primary share-social"><i class="fa fa-whatsapp"></i>
                                </button>
                                <button data-url="https://twitter.com/intent/tweet?url=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="btn btn-primary share-social"><i class="fa fa-twitter"></i>
                                </button>
                                <button data-url="https://www.linkedin.com/shareArticle?mini=true&url=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif&title={{$video->title}}" class="btn btn-primary share-social"><i class="fa fa-linkedin"></i>
                                </button>
                                <button data-url="https://pinterest.com/pin/create/button/?url=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="btn btn-primary share-social"><i class="fa fa-pinterest"></i>
                                </button>
                                <button data-url="https://www.tumblr.com/share/link?url=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="btn btn-primary share-social"><i class="fa fa-tumblr"></i>
                                </button>
                                <button data-url="https://www.reddit.com/submit?url=@if($user->user_extra->	distribution_type=='embedded'){{route('embed_video',$video->video_id)}}@else {{route('directory_by_username',[request('slug'),request('video_id',$video->video_id)])}} @endif" class="btn btn-primary share-social"><i class="fa fa-reddit"></i></button>
                            </div>
                        </div>
                                ...
                                <div class="modal-body">
                                    <form method="POST" action="{{route('to_user')}}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="message" required class="form-control" id="contact_message" rows="3"></textarea>
                                        </div>
                ...
                <div class="row video-list">
                    ...
                    <div class="col-12 m-0 p-0 my-2" id="related_video">
                        <a href="{{route('directory_by_username',[$user->username,$related_video->video_id])}}">
                            <div class="p-2 shadow-lg bg-white rounded video-thumb overlay" style="background-image: url({{asset('storage/'.$related_video->thumbnail)}});background-size: cover;height:200px;">
                                <div class='play_hover_btn' style="top: 50%;left: 50%;position: absolute;transform: translate(-50%, -50%);">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-white feather feather-play-circle">
    ...
    <div class="row">
        <div class="col-12">
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li class="nav-item" role="Report">
                    <a class="nav-link" id="contact-tab" data-toggle="tab" href="#report_message" role="tab" aria-controls="report_message" aria-selected="false">Report</a>
                </li>
            </ul>
            <div class="tab-content p-3 card">
                ...
                {{-- aria-valuenow="{{$rating[5]}}" aria-valuemin="0" aria-valuemax="100">
            </div>--}}
            {{-- </div>--}}
            {{-- </div>--}}
            {{-- <div class="d-flex align-items-center my-2">--}}
            ...
            {{-- aria-valuenow="{{$rating[4]}}" aria-valuemin="0" aria-valuemax="100">
        </div>--}}
        {{-- </div>--}}
        {{-- </div>--}}
        ...
        {{-- aria-valuenow="{{$rating[3]}}" aria-valuemin="0" aria-valuemax="100">
    </div>--}}
    {{-- </div>--}}
    {{-- </div>--}}
    {{-- <div class="d-flex align-items-center my-2">--}}
    ...
    {{-- aria-valuenow="{{$rating[2]}}" aria-valuemin="0" aria-valuemax="100">
</div>--}}
{{-- </div>--}}
{{-- </div>--}}
{{-- <div class="d-flex align-items-center my-2">--}}
...
<div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
    ...
    <form method="POST" action="{{route('to_user')}}">
        ...
        <div class="form-group">
            ...
            <textarea name="message" class="form-control" id="message2" rows="3" placeholder="Message here"></textarea>
        </div>
...
<script>
    function initMap() {
        ...
        let position = {
            lat: {
                {
                    $user - > user_extra - > location_latitude
                }
            },
            lng: {
                {
                    $user - > user_extra - > location_longitude
                }
            }
        };
        var map = new google.maps.Map(
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAm4Wvmd2nIeaFQCdhAsxbiSXgBsibDolc&libraries=places&callback=initMap">
</script>
